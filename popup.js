    ...
    function updateDictionaryList() {
        chrome.storage.local.get('dictionaryList', function (result) {
            ...
            dictionaryList.forEach(function (dict) {
                ...
                dictDiv.innerHTML = `
      <input type="text" class="dictionary-name-input" data-name="${dict.name}" value="${dict.name}" ${dict.name === 'default' ? 'disabled' : ''}>
      <div>
        <button class="toggle-dict" data-name="${dict.name}">${dict.enabled ? 'Disable' : 'Enable'}</button>
        ${dict.name !== 'default' ? `<button class="remove-dict" data-name="${dict.name}">Remove</button>` : ''}
      </div>
